...
for(c=0;c<3;c++){
obuf[c]
[WOADR(bx,by,part_x*part_wx+xx-part_ox,part_y*part_wy+yy-part_oy)]
  = dbuf[c][ADR(xx,yy)];
}}}
}
...
if(!strstr(st,"IGNORE_COMMENT") && st[0]!=0){
nuke=strstr(st,"/*");if(nuke)*nuke=0;
fprintf(sfp,"#define TILEP_%s_%s %d\n", part_name, st, part_n+1);
