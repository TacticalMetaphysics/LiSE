...
class Rule(object):
    def __init__(self, orm, name, actions=[], prereqs=[]):
        ...
        self.actions = self.FunList(self, 'actions', actions)
        self.prereqs = self.FunList(self, 'prereqs', prereqs)

    def __call__(self, *args):
        for prereq in self.prereqs:
            if not prereq(self, *args):
                return []
        return [action(self, *args) for action in self.actions]
