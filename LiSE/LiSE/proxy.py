...
class EngineHandle(object):
    ...
    def get(self):
        return self._real.json_load(self._q.get())
    ...
    def listen_to_universals(self):
        ...
        def dispatch_var(branch, tick, mapping, key, value):
            if (branch, tick) == (self.branch, self.tick):
                self.put(('universal', branch, tick, key, value))
        ...
        def dispatch_vars(oldb, oldt, newb, newt):
            ...
            for (k, oldv) in old_universals.items():
                ...
                if oldv != newv:
                    self.put(('universal', newb, newt, k, newv))
    ...
    def listen_to_character_map(self):
        ...
        def charactered(charmap, k, v):
            if v is None:
                self.put(('character_map', k, False))
            else:
                self.put(('character_map', k, True))
            ...
            if (b, t) == (self.branch, self.tick):
                self.put(
                    (
                        ...
                        b, t,
                        char.name,
                        k, self.unwrap_character_stat(char.name, k, v)
                    )
                )
                ...
                if k not in character.stat:
                    ...
                    self.put(
                        (
                            ...
                            newb, newt,
                            charn,
                            k, self.unwrap_character_stat(charn, k, v)
                        )
                    )
                seen.add(k)
            ...
            if oldv != newv:
                self.put(
                    (
                        ...
                        newb, newt,
                        charn,
                        statn, self.unwrap_character_stat(charn, statn, newv)
                    )
                )
    ...
    def listen_to_thing_map(self, charn):
        ...
        def put_thing(b, t, mapping, thingn, v):
            ...
            if (b, t) != (self.branch, self.tick):
                return
            self.put(('thing_extant', b, t, charn, thingn, v is not None))
    ...
    def listen_to_place_map(self, charn):
        ...
        def put_place(b, t, mapping, placen, v):
            ...
            if (b, t) != (self.branch, self.tick):
                return
            self.put(('place_extant', b, t, charn, placen, v is not None))
            ...
            if (b, t) == (self.branch, self.tick):
                self.put(
                    (
                        ...
                        b, t,
                        charn, noden,
                        k, self.unwrap_node_stat(charn, noden, k, v)
                    )
                )
            ...
            if oldv != newv:
                self.put(
                    (
                        ...
                        newb, newt,
                        charn, noden,
                        statn, self.unwrap_node_stat(charn, noden, statn, newv)
                    )
                )
            ...
            if (b, t) == (self.branch, self.tick):
                self.put(
                    (
                        ...
                        b, t,
                        char, a, b,
                        k, self.unwrap_portal_stat(char, a, b, k, v)
                    )
                )
        ...
        def put_stat(b, t, portal, k, v):
            ...
            self.put(
                (
                    ...
                    b, t,
                    charn, a, b,
                    statn, self.unwrap_portal_stat(charn, a, b, statn, v)
                )
            )
            ...
            if oldv != newv:
                self.put(
                    (
                        ...
                        newv, newt,
                        charn, a, b,
                        statn, self.unwrap_portal_stat(charn, a, b, statn, newv)
                    )
                )
    ...
    def on_node_stat(self, char, node, k, v):
        self.put(('on_node_stat', char, node, k, v))
...
class ThingMapProxy(CachingProxy):
    ...
    def unlisten(self, fun):
        self.engine.thing_map_unlisten(
            self.character.name, fun
        )
        return fun
...
class PlaceMapProxy(CachingProxy):
    ...
    def unlisten(self, fun):
        self.engine.place_map_unlisten(
            self.character.name, fun
        )
        return fun
...
class CharSuccessorsMappingProxy(CachingProxy):
    ...
    def listener(self, fun):
        self.engine.portal_map_listener(self.name, fun)

    def unlisten(self, fun):
        self.engine.portal_map_unlisten(self.name, fun)
...
class RuleBookProxy(MutableSequence):
    ...
    def listener(self, fun):
        self.engine.rulebook_listener(self.name, fun)
        return fun

    def unlisten(self, fun):
        self.engine.rulebook_unlisten(self.name, fun)
        return fun
...
class CharacterProxy(MutableMapping):
    ...
    def mute(self):
        self.engine.mute_character(self.name)

    def unmute(self):
        self.engine.unmute_character(self.name)
...
class StringStoreProxy(MutableMapping):
    ...
    def lang_listener(self, f):
        self.engine.lang_listener(f)
        return f

    def lang_unlisten(self, f):
        self.engine.lang_unlisten(f)

    def listener(self, fun=None, string=None):
        if None not in (fun, string):
            self.engine.string_listener(string, fun)
            return fun
        elif string is None:
            self.engine.strings_listener(fun)
            return fun
        else:
    ...
    def unlisten(self, fun=None, string=None):
        if None not in (fun, string):
            self.engine.string_unlisten(string, fun)
        elif string is None:
            self.engine.strings_unlisten(fun)
        else:
...
class EngineProxy(AbstractEngine):
    ...
    def poll_changes(self, num_changes=None):
        try:
            ...
            while num_changes is None or n < num_changes:
                self._process_change(self.json_load(self._q.get(False)))
                n += 1
        except Empty:
            return
    ...
    def _process_change(self, v):
        ...
        if not isinstance(v, tuple):
            raise TypeError(
                "Can only process changes in tuple form, not {}".format(
        ...
        if typ == 'language':
            ...
            if (
                ...
                for fun in self._portal_listeners[charn][a][b]:
                    fun(branch, tick, portal, stat, val)
            if (
                    charn in self._portal_stat_listeners and
                    a in self._portal_stat_listeners[charn] and
                    b in self._portal_stat_listeners[charn][a] and
