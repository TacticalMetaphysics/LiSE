    ...
    def _apply_diff(self, diff):
        ...
        for (node, nodediff) in diff['node_stat'].items():
            if node in self.node:
                ...
                self.engine.warning("CharacterProxy({}): node {} changed and deleted".format(
                    self.name,  node
                ))
        for (orig, destdiff) in diff['portal_stat'].items():
            for (dest, portdiff) in destdiff.items():
               if orig in self.portal and dest in self.portal[orig]:
                    ...
                    self.engine.warning("CharacterProxy({}): portal {}->{} changed and deleted".format(
                        self.name,  orig,  dest
                    ))
...
class EngineProxy(AbstractEngine):
    ...
    def next_tick(self, char=None, cb=None):
        ...
        def upon_next_tick(func_name,  args,  result):
            self._inc_tick()
            self._upd_char_cache(args[0],  result)
            if cb:
                cb(func_name,  args,  result)
        if char:
            Thread(
