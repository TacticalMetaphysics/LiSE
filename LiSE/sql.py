...
poll_node_rules = (
    ...
    "LEFT OUTER JOIN "
    "(SELECT character, node, rulebook, rule, 1 AS handled "
    ...
    "handle.rulebook=node_rulebook.rulebook AND "
...
poll_portal_rules = (
    "SELECT "
    "portal_rulebook.character, "
    "portal_rulebook.nodeA, "
    "portal_rulebook.nodeB, "
    "portal_rulebook.idx, "
    "active_rules.rule, "
    "active_rules.active, "
    "handle.handled "
    "FROM portal_rulebook JOIN active_rules ON "
    "portal_rulebook.rulebook=active_rules.rulebook "
    "JOIN "
    "(SELECT rulebook, rule, branch, MAX(tick) AS tick "
    "FROM active_rules WHERE "
    "branch=? AND "
    ...
    "AND active_rules.tick=hitick.tick "
    "LEFT OUTER JOIN rulebooks "
    "ON rulebooks.rulebook=portal_rulebook.rulebook "
    "AND rulebooks.rule=active_rules.rule "
    "LEFT OUTER JOIN "
    "(SELECT "
    "character, "
    ...
    "rule, "
    "1 AS handled FROM "
    "portal_rules_handled WHERE branch=? AND tick=?) "
    "AS handle ON "
    "handle.character=portal_rulebook.character AND "
    "handle.nodeA=portal_rulebook.nodeA AND "
    "handle.nodeB=portal_rulebook.nodeB AND "
    "handle.idx=portal_rulebook.idx AND "
    "handle.rulebook=portal_rulebook.rulebook AND "
    "handle.rule=active_rules.rule "
    "WHERE handle.handled IS NULL "
    "ORDER BY "
    "portal_rulebook.character, "
