...
class QueryEngine(gorm.query.QueryEngine):
    ...
    def active_rules(self, tbl, character, rulebook, branch, tick):
        character = json_dump(character)
        seen = set()
        for (b, t) in self.active_branches(branch, tick):
            for (rule, active) in self.sql(
                    'active_rules_fmt', character, rulebook, b, t, tbl=tbl
            ):
                if active and rule not in seen:
                    yield rule
                seen.add(rule)

    def active_rule(self, tbl, character, rulebook, rule, branch, tick):
        character = json_dump(character)
        for (b, t) in self.active_branches(branch, tick):
            for (active,) in self.sql(
                    'active_rule_fmt', character, rulebook, rule, b, t, tbl=tbl
            ):
                return bool(active)
        return False
    ...
    def get_rulebook(self, rulemap, character):
        character = json_dump(character)
        for (book,) in self.sql(
                'rulebook_get_fmt', character, rulemap=rulemap
        ):
            return book
        raise KeyError("No rulebook")

    def upd_rulebook(self, rulemap, character):
        return self.sql('upd_rulebook_fmt', character, rulemap=rulemap)
